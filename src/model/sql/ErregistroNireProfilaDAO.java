...
public class ErregistroNireProfilaDAO {
	 ...
	 */
	public static void erregistroaFree(Erabiltzailea e_free)
			throws SQLException, ClassNotFoundException, ErabiltzaileBalidazioaException {
		Konexioa.konexioaIreki();
		...
		try {
			...
			} else if (e_free.getIzena().isEmpty() && e_free.getErabiltzailea().isEmpty()
					&& e_free.getPasahitza().isEmpty()) {
	 ...
	 * @throws ErabiltzaileBalidazioaException
	 */
	public static void erregistroaPremium(Erabiltzailea e_premium)
			throws SQLException, ClassNotFoundException, ErabiltzaileBalidazioaException {
		Konexioa.konexioaIreki();
		...
		try {
			...
			Konexioa.query.executeUpdate(SQLquery);
			JOptionPane.showMessageDialog(null, "Ondo erregistratu egin zara Premium bezeroa bezala.",
					"Erregistroa [Premium]", JOptionPane.INFORMATION_MESSAGE);
		} catch (SQLException e) {
			//
			if (e.getSQLState().equals("23000") && e.getErrorCode() == 1062) {
				throw new ErabiltzaileBalidazioaException();
	...
	public static void updateNireProfilaDatuak(Erabiltzailea erab) throws SQLException {
		Konexioa.konexioaIreki();
		String SQLquery = "UPDATE bezeroa SET Izena = '" + erab.getIzena() + "', Abizena = '" + erab.getAbizena()
				+ "', Hizkuntza = '" + erab.getHizkuntza() + "', Erabiltzailea = '" + erab.getErabiltzailea()
				+ "', Pasahitza = '" + erab.getPasahitza() + "', Jaiotza_data = '"
				+ View_metodoak.dateToString(erab.getJaiotze_data()) + "' WHERE Erabiltzailea = '"
				+ erab.getErabiltzailea() + "';";
		Konexioa.query.executeUpdate(SQLquery);
		if (Konexioa.query.executeUpdate(SQLquery) == 1) {
