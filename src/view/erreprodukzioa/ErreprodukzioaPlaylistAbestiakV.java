...
public class ErreprodukzioaPlaylistAbestiakV extends JFrame {
	 ...
	 * @throws SQLException 
	 */
	public ErreprodukzioaPlaylistAbestiakV(ArrayList<Abestia> abestiLista, Playlist playlist, Artista artista, Album album, int indexAbesti) throws SQLException {
		index = indexAbesti;
		...
		if (playlist.getTitulua().equals("Gustokoen zerrenda")) {
			artistakAbestiErreproduzitzen = PlaylistAbestiakDAO.ArtistakGustokoaPlaylistKargatu(abestiErreproduzitzen.getIdAudio());
			albumAbestiErreproduzitzen = PlaylistAbestiakDAO.albumaGustokoaPlaylistKargatu(abestiErreproduzitzen.getIdAudio());
			System.out.println(artistakAbestiErreproduzitzen);
			for (int i = 0; i < abestiLista.size(); i++) {
				if (SesioAldagaiak.e_premium) {
					setTitle(abestiLista.get(index).getTitulua() + " - " + artistakAbestiErreproduzitzen.getIzena() + " | Erreprodukzioa · JPAM Music PREMIUM");
				} else {
					setTitle(abestiLista.get(index).getTitulua() + " - " + artistakAbestiErreproduzitzen.getIzena() + " | Erreprodukzioa · JPAM Music FREE");
				}
			}
		} else {
			artistakAbestiErreproduzitzen = PlaylistAbestiakDAO.ArtistakPlaylistKargatu(abestiErreproduzitzen.getIdAudio());
		...
		ImageIcon argazkiaAlbum = new ImageIcon(abestiErreproduzitzen.getIrudia().getBytes(1, (int) abestiErreproduzitzen.getIrudia().length()));
			...
			public void mouseClicked(MouseEvent e) {
				if (clipLehena.isRunning()) {
					if (clipLehena.isActive()) {
						...
						lblIraupena.setText("0" + iMin + ":" + secondsInt + " / " + abestiErreproduzitzen.getIraupena());
